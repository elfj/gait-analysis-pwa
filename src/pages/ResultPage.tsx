...
import { exportPdfReport } from '@/lib/report/pdfExporter';
...
/** Export one rendered result report to PDF. */
export type ResultPdfExporter = (
  element: HTMLElement,
  result: GaitAnalysisResult,
) => Promise<void>;

const defaultPdfExporter: ResultPdfExporter = async (element, result) => {
  await exportPdfReport({ element, result });
};

/** Render the result dashboard. */
export function ResultPage({
  pdfExporter = defaultPdfExporter,
  repository = defaultRepository,
}: {
  /** Optional PDF exporter override for tests. */
  ...
  if (status === 'loading') {
    return (
      <ResultShell assessmentId={assessmentId}>Loading result...</ResultShell>
    );
  }
  ...
  async function handleDownloadPdf(): Promise<void> {
    ...
    try {
      await pdfExporter(reportRef.current, result);
      setPdfStatus('idle');
    } catch {
      setPdfStatus('error');
    }
  ...
  return (
    <ResultShell assessmentId={assessmentId}>
                ...
                <table className="w-full min-w-[36rem] text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {createMetricRows(result).map((row) => (
                      <tr key={row.label}>
                        <td className="py-2 font-medium text-slate-950">
                          {row.label}
                        </td>
            ...
            result.quality.warnings.length > 0 ? (
              <section className="rounded-md border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900">
                ...
                <ul className="mt-3 space-y-1">
                  {[...result.quality.warnings, ...result.confidenceFlags].map(
                    (warning) => (
      ...
      <div className="flex flex-wrap gap-3">
        <button
          className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
        ...
        >
          ...
          {pdfStatus === 'exporting' ? 'Exporting PDF...' : 'Download PDF'}
        </button>
        <Link
...
function createMetricRows(
  ...
  return [
    {
      label: 'Stride time mean',
      unit: 's',
      value: formatNumber(st.strideTimeMean, 2),
    },
    {
      label: 'Stride time CV',
      unit: '%',
      value: formatNumber(st.strideTimeCV, 1),
    },
    {
      label: 'Left step length',
      unit: 'm',
      value: formatNumber(st.stepLengthLeft, 2),
    },
    {
      label: 'Right step length',
      unit: 'm',
      value: formatNumber(st.stepLengthRight, 2),
    },
    { label: 'Step width', unit: 'm', value: formatNumber(st.stepWidth, 2) },
    {
      label: 'Left stance',
      unit: '%',
      value: formatNumber(st.stancePhasePctLeft, 1),
    },
    {
      label: 'Right stance',
      unit: '%',
      value: formatNumber(st.stancePhasePctRight, 1),
    },
    {
      label: 'Double support',
      unit: '%',
      value: formatNumber(st.doubleSupportPct, 1),
    },
  ];
}
